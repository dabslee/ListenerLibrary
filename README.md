...
*   **Offline Listening (PWA):** Save individual tracks (track menu → "Save for offline") or a whole playlist (playlist page → "Save for offline") to your device, with live download progress shown on track icons and on the Downloads page. ListenerLibrary is a Progressive Web App with a service worker, so saved tracks — and the app itself — remain playable with no internet connection. Manage or remove saved tracks on the **Downloads** page (under your account menu). Listening progress made while offline is stored on-device and synced back automatically when connectivity returns, with conflicts resolved newest-wins against progress from other devices. Works on mobile Safari (iOS) and other modern browsers; add the site to your home screen for a full-screen, app-like experience.
